...
test('by year, v1 (ctrl + left/right arrows)', function(){
    ...
    datesEqual(this.dp.viewDate, UTCDate(2017, 2, 31));
    datesEqual(this.dp.date, UTCDate(2017, 2, 31));
    target = this.picker.find('.datepicker-days thead th.datepicker-switch');
    equal(target.text(), 'March 2017', 'Title is "March 2017"');
});
...
test('by year, from leap day', function(){
    ...
    // Both updated on keyboard navigation; graceful month-end
    datesEqual(this.dp.viewDate, UTCDate(2017, 1, 28));
    datesEqual(this.dp.date, UTCDate(2017, 1, 28));
    target = this.picker.find('.datepicker-days thead th.datepicker-switch');
    equal(target.text(), 'February 2017', 'Title is "February 2017"');
});
